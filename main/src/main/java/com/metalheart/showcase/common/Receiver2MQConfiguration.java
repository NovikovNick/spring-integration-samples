...
@Configuration
public class Receiver2MQConfiguration {

    public static final String PREFIX = "receiver2-";
    ...
    public Queue receive2rQueue() {
        return new Queue(QUEUE_NAME);
    }

    @Bean
    @Qualifier(EXCHANGE_NAME)
    public Exchange receiver2Exchange() {
        return new DirectExchange(EXCHANGE_NAME);
    }

    @Bean
    @Qualifier(BINDING_NAME)
    public Binding receiver2Binding(@Qualifier(QUEUE_NAME) Queue queue,
    ...
    public RabbitTemplate receiver2Template(ConnectionFactory connectionFactory) {
